...
# Choose maximum random displacement
max_rand_dist_x = int(dist_x / 2)
max_rand_dist_y = int(dist_y / 2)


# Draw each star to the screen
for num_row in range(num_rows):
    for num_star in range(num_stars_in_row):
        star_x = (edge_dist_x + num_star * (star_width + dist_x) 
                + randint(-1 * max_rand_dist_x, max_rand_dist_x))
        star_y = (edge_dist_y + num_row * (star_height + dist_y) 
                + randint(-1 * max_rand_dist_y, max_rand_dist_y))
        screen.blit(star, (star_x, star_y))

# Load printed screen from pygame buffer
pygame.display.flip()

# Event loop to keep program from ending until user quits
while True:
